...
function fakeMatter(doc) {
  const counts={engines:0,clears:0,runnerStops:0,renderStops:0,balls:0,radii:[]};
  const Matter={Engine:{create:()=>{counts.engines++;return {world:{}};},clear:()=>counts.clears++},
    ...
    Bodies:{rectangle:()=>({}),circle:(x,y,r,options)=>{if(!options?.isStatic){counts.balls++;counts.radii.push(r);}return {};}},Composite:{add(){},clear(){}},};
...
test('136 Matter initializes only on Play, stops, replays and never reruns analytics',async()=>withUI(async h=>{
  ...
  try{visual.render(visualView(h.store));assertEqual(counts.engines,0);h.q('#visual-play').click();
    assertEqual(counts.engines,1);assertEqual(counts.balls,4);assertEqual(h.q('#visual-stop').hidden,false);
    assertDeepEqual(counts.radii,[5,7,9,5]);assert(h.q('.visual-score').textContent.includes('4 / 200 balls'));
...
test('141 a rounded even matchup has a neutral game day headline',async()=>withUI(async h=>{
  ...
  result.scenarios.at(-1).probabilitySummary.teamA.mean=.50001;
  result.scenarios.at(-1).probabilitySummary.teamB.mean=.49999;
  const summary=buildGameDaySummary(result);
  assertEqual(summary.headline,'Too close to call: 50.0% each.');
  assert(!summary.body.includes('key statistical matchup'));
}));
