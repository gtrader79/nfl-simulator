...
export function createRenderer({ root, metricCatalog = METRIC_CATALOG }) {
  ...
  function base(view) {
    ...
    for (const [direction, offense, defense] of [
      ...
      const chartHost=node('div',undefined,'chart-host contribution-chart');
      chartHost.dataset.chart=offense===view.teamA?'team-a-offense':'team-b-offense';
      const chartStatus=node('p',undefined,'muted');chartStatus.dataset.chartStatus='';
      card.append(chartHost,chartStatus,table(['Metric', 'Contribution'], rows, 'Base matchup contributions'));
  ...
  return Object.freeze({
    render(state, view) {
      ...
      find('probability-chart-region').hidden=!view.simulationResult;
