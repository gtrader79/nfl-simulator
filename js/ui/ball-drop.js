...
export function createBallDrop({root,Matter,randomSource=Math.random,ResizeObserverImpl=globalThis.ResizeObserver,
  ...
  function halt(clear=false) {
    ...
    if(clear) {
      ...
      engine=null;runner=null;render=null;host.replaceChildren();
    }
  }
  ...
  function drop() {
    ...
    try {
      ...
      const x=lane*width/2+width/4+(randomSource()-.5)*width*.18;
      const body=Matter.Bodies.circle(x,24,7,{restitution:.55,friction:.15,render:{fillStyle:team.colors.primary,strokeStyle:'#ffffff',lineWidth:1}});
      Matter.Composite.add(engine.world,body);counts[side]++;count++;
      timer=setTimeout(count<80?drop:finish,count<80?80:3500);
  ...
  function playVisual() {
    ...
    try {
      ...
      message.textContent='Playing the previous analytical result. Ball counts do not change the prediction.';
  ...
  return Object.freeze({
    ...
    destroy(){if(destroyed)return;destroyed=true;halt(true);play.removeEventListener('click',playVisual);stop.removeEventListener('click',stopVisual);motionQuery?.removeEventListener?.('change',motionChanged);root.ownerDocument.removeEventListener('visibilitychange',visibilityChanged);},
  });
}
